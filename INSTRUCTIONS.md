...
The script uses **interactive authentication**.
...
4. It uses the same interactive admin identity for data collection.
...
Even with `-AdminUPN`, the script still expects an interactive sign-in experience. If your environment enforces MFA or Conditional Access, you must be able to complete that prompt.
...
The script can be made more automation-friendly, but it is still designed around interactive authentication.
...
- authentication still requires interactive Exchange Online / Security & Compliance sign-in
...
Not as a fully unattended solution in its current form. The script is built around interactive authentication.
